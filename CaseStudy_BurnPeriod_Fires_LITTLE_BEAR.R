...
library(readxl)
SITdf <- read_excel("data/SIT209/LittleBear_2012_SIT209.xlsx", 
...
tempDF <- subset(tempDF, date >= (SITstart) & date <= (SITend))
...
tempDF <- subset(tempDF, DateAZ >= (SITstart) & DateAZ <= (SITend))
...
temp<-subset(SITthin,INCIDENT_NAME == "Little Bear")
...
p1<-ggplot(subset(SITthin, INCIDENT_NAME == "Little Bear"), aes(REPORT_DATE, AREA)) +
  geom_line()+
...
kable(SITthin[,c("REPORT_DATE","P_CONTAIN","AREA","OBS_FIRE_BEHAVE","GROWTH_POTENTIAL","TARGETS_MET","bhrs20","bhrs20_med_anom")]) %>%
  ...
  save_kable("Little_bear_summary.png")

SITthin[,c("REPORT_DATE","P_CONTAIN","AREA","OBS_FIRE_BEHAVE","GROWTH_POTENTIAL","TARGETS_MET","bhrs20","bhrs20_med_anom")] %>%
  kable() %>%
