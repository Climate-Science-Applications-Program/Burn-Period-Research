# Burn period case study fires with SIT-209 data
...
##### load SIT-209 data ----
library(readxl)
SITdf <- read_excel("data/SIT209/Cave_Creek_2005_209data.xlsx",
  col_types = c(
    "date", "text", "text",
...
SITthin$dayAcres[SITthin$dayAcres < 0] <- 0
 ...
 temp<-subset(SITthin,INCIDENT_NAME == "Cave Creek Complex")
...
# get hourly RAWS data from station  
 ...
 # read in RAWS file
 tempRAWS<-read.csv(file_list[grep(tempDF$StationNum[1], file_names, value = FALSE)])
 ...
 elev<-elevatr::get_elev_point(data.frame(x=tempDF$LONGITUDE,y=tempDF$LATITUDE),prj = "EPSG:4326", src = "aws")
...
p1<-ggplot(subset(SITthin, INCIDENT_NAME == "Cave Creek Complex"), aes(REPORT_DATE, AREA)) +
    geom_line()+
...
p1<-ggplot(subset(SITthin, INCIDENT_NAME == "Cave Creek Complex"), aes(REPORT_DATE, AREA)) +
  geom_line()+
  ggtitle("Cave Creek Complex Wildfire")
p2 <- ggplot(temp, aes(REPORT_DATE, dayAcres)) +
  geom_line()
...
plot_grid(p1,p2,p3,p4, ncol = 1, align = "hv", axis = "b")


# create a table
library(knitr)
library(kableExtra)
kable(temp[,c("REPORT_DATE","P_CONTAIN","AREA","OBS_FIRE_BEHAVE","GROWTH_POTENTIAL","TARGETS_MET","bhrs20","bhrs20_med_anom")]) %>%
  #kable_styling(latex_options = "striped") %>%
  kable_styling(full_width = T, html_font = "Cambria", font_size = 12) %>%
  save_kable("Cave_Creek_summary.png")
